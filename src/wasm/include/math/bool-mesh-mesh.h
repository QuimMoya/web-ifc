...
namespace webifc
{
    ...
    IfcGeometry AddGeometryFace(IfcGeometry geometry, uint32_t a, uint32_t b, uint32_t c, uint32_t j)
    {
        glm::dvec3 pa = geometry.GetPoint(a);
        glm::dvec3 pb = geometry.GetPoint(b);
        glm::dvec3 pc = geometry.GetPoint(c);
        glm::dvec3 pj = geometry.GetPoint(j);

        glm::dvec3 va = pa - pb;
        glm::dvec3 vb = pb - pc;
        glm::dvec3 ga = glm::normalize(glm::cross(va, vb));

        va = pj - pa;
        vb = pa - pb;
        glm::dvec3 gb = glm::normalize(glm::cross(va, vb));

        double side = glm::dot(ga, gb);
        if (side >= 0.9)
        {
            geometry.AddFace(a, b, j);
            geometry.AddFace(b, c, j);
        }
        else
        {
            geometry.AddFace(b, a, j);
            geometry.AddFace(c, b, j);
        }

        return geometry;
    }

    IfcGeometry removeNakedEdges(IfcGeometry geometry)
    {
        // Some solids have triangles with naked edges
        // these naked edges cause manifold to fail
        // These triangles must be divided
        ...
        for (int i = 0; i < geometry.numFaces; i++)
        {
            ...
            for (int j = 0; j < geometry.numPoints; j++)
            {
                ...
                if (d1 > 1e-8 && d2 > 1e-8 && std::abs(d1 + d2 - d3) < 1e-8)
                {
                    geometry = AddGeometryFace(geometry, b, c, a, j);
                    geometry.indexData.erase(geometry.indexData.begin() + (i * 3), geometry.indexData.begin() + (i * 3) + 3);
                    i--;
                    geometry.numFaces--;
                    break;
                }
                ...
                if (d1 > 1e-8 && d2 > 1e-8 && std::abs(d1 + d2 - d3) < 1e-8)
                {
                    geometry = AddGeometryFace(geometry, a, b, c, j);
                    geometry.indexData.erase(geometry.indexData.begin() + (i * 3), geometry.indexData.begin() + (i * 3) + 3);
                    i--;
                    geometry.numFaces--;
                    break;
                ...
                if (d1 > 1e-8 && d2 > 1e-8 && std::abs(d1 + d2 - d3) < 1e-8)
                {
                    geometry = AddGeometryFace(geometry, c, a, b, j);
                    geometry.indexData.erase(geometry.indexData.begin() + (i * 3), geometry.indexData.begin() + (i * 3) + 3);
                    i--;
                    geometry.numFaces--;
                    break;
    ...
    manifold::Manifold *geom2manifold(const IfcGeometry &geometry, uint32_t expressID)
    {
        ...
        if (manifold->Status() != manifold::Manifold::Error::NO_ERROR)
        {
            ...
            // std::string fileName = "error_EID_" + std::to_string(expressID) + ".obj";
            // std::wstring fileName2(fileName.length(), L' '); // Make room for characters
            // std::copy(fileName.begin(), fileName.end(), fileName2.begin());
            // DumpIfcGeometry(geometry, fileName2);

           return NULL;
        }

        // std::string fileName = "correct_EID_" + std::to_string(expressID) + ".obj";
        // std::wstring fileName2(fileName.length(), L' '); // Make room for characters
        // std::copy(fileName.begin(), fileName.end(), fileName2.begin());
        // DumpIfcGeometry(geometry, fileName2);
    ...
    IfcGeometry boolMultiOp_Manifold(const std::vector<IfcGeometry> &firstGeoms, const std::vector<IfcGeometry> &secondGeoms, uint32_t expressID)
    {

        IfcGeometry resultGeom;

        // create mainBody
        manifold::Manifold *mainBody;
        bool first = true;

        for (auto &geom : firstGeoms)
        {
            manifold::Manifold *geomManifold = geom2manifold(removeNakedEdges(geom), expressID);

            if (first)
            {
                if (geomManifold)
                {
                    mainBody = geomManifold;
                    first = false;
                }
            }
            else
        ...
        if (performedBoolean && !first && mainBody->IsManifold() && combinedHole.IsManifold())
        {
            (*mainBody) -= combinedHole;

            // convert back to IfcGeometry
            manifold::Mesh resultMesh = mainBody->GetMesh();
