...
void StreamAllMeshes(uint32_t modelID, emscripten::val callback)
{
    ...
    {
        if (type == ifc2x4::IFCOPENINGELEMENT || type == ifc2x4::IFCSPACE)
        {
            continue;
        }
...
std::vector<webifc::IfcAlignment> LoadAllAlignments(uint32_t modelID)
{
    ...
    for (int i = 0; i < elements.size(); i++)
    {
        ...
        alignment.origin = geomLoader->origin;
...
EMSCRIPTEN_BINDINGS(my_module)
{
    ...
    emscripten::value_object<webifc::IfcAlignment>("IfcAlignment")
        .field("origin", &webifc::IfcAlignment::origin)
    ...
    emscripten::function("LoadAllAlignments", &LoadAllAlignments);
